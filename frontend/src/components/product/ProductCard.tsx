 ...
 * noise — so the still state is a photograph, a rule and set type, and every
 * affordance (quick add, wishlist, the label reveal) is held back until hover.
 * The one persistent flourish is the index number, which ties the grid to the
    ...
    >
      <Link href={`/product/${product.slug}`} className="block">
        <div className="relative aspect-[4/5] overflow-hidden bg-ivory-deep">
          ...
          {/* Actions slide in from the bottom edge on a shared timing. */}
          <div className="absolute inset-x-0 bottom-0 z-10 flex translate-y-full items-stretch opacity-0 transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:translate-y-0 group-hover:opacity-100">
            <button
              onClick={handleQuickAdd}
              className="flex flex-1 cursor-pointer items-center justify-center gap-2 bg-noir/90 py-3.5 text-[10px] uppercase tracking-[0.2em] text-ivory backdrop-blur transition-colors hover:bg-deep-rose"
            >
            ...
            <button
              ...
              className="flex w-12 cursor-pointer items-center justify-center border-s border-ivory/15 bg-noir/90 text-ivory backdrop-blur transition-colors hover:bg-deep-rose"
            >
